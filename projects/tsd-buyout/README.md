# TSD Buyout

**Status:** In Progress  
**Owner:** Rick Snide  
**Started:** 2026-05-25

## What this is

Strategic initiative to evaluate and execute a buyout of TSD (The Support Desk), where Rick currently serves as Director. This project tracks all planning, financial analysis, stakeholder alignment, and execution milestones.

## Key files

| File | Purpose |
|------|---------|
| [tracker.md](tracker.md) | Milestones, tasks, and open questions |
| [decisions.md](decisions.md) | Decision log — what was decided and why |
| [financials.md](financials.md) | Financial model notes, terms, assumptions |
| [stakeholders.md](stakeholders.md) | Who's involved and their role/stance |

## Quick links

- Decisions log: [decisions/log.md](../../decisions/log.md)
- Connections: [connections.md](../../connections.md)
